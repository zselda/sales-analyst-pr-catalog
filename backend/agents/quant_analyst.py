...
Refactored to use BaseAgent for tracing and error isolation.
INCLUDES BANKING INTELLIGENCE: Cash Conversion Cycle, Insider Lending, 
Check Risk Ratios, and advanced LLM Prompting for Credit Analysis.

DYNAMIC MAPPING: Account code descriptions (hesap_kodu_aciklama) are 
extracted dynamically from each Mizan document.
...
def _aggregate_hierarchy(df: pd.DataFrame, code: str, tree: dict,
    ...
    return {
        "period_borc": period_borc, "period_alacak": period_alacak,
        "period_movement": period_borc - period_alacak,
        "closing_borc_bakiye": closing_b, "closing_alacak_bakiye": closing_a,
        "closing_balance": closing_b - closing_a,
        "leaf_count": len(leaf_codes), "leaf_codes": leaf_codes,
        "reported_value": reported_value, "validation_status": validation_status,
    }
...
class QuantAnalystAgent(BaseAgent):
    ...
    def execute(self, state: dict) -> dict:
        ...
        # ── LOCAL CALCULATIONS (PROFITABILITY) ──
        revenue_600 = abs(bal("600"))
        cogs_620 = abs(bal("620"))
        gross_profit = revenue_600 - cogs_620
        gross_margin = ((gross_profit) / revenue_600 * 100) if revenue_600 else 0
        ...
        # EBITDA Proxy (Adding back Depreciation if detectable in 257/268 movements)
        depreciation_257_268 = abs(bal("257")) + abs(bal("268")) 
        ebitda_proxy = operating_profit + depreciation_257_268

        # ── LIQUIDITY & WORKING CAPITAL ──
        ca_codes = df[df["category"] == "Current Assets"]["account_code"].tolist()
        current_assets = float(df[df["category"] == "Current Assets"]["debit"].sum())
        
        # Calculate Total Assets for Banking Ratios
        total_assets = current_assets + float(df[df["category"] == "Non-Current Assets"]["debit"].sum()) if "Non-Current Assets" in df["category"].values else current_assets
        ...
        # Efficiency & Cash Conversion Cycle (Nakit Döngüsü)
        # dynamic period_days is used
        trade_recv_120 = abs(bal("120"))
        trade_recv_121 = abs(bal("121"))
        trade_receivables = trade_recv_120 + trade_recv_121
        collection_period = (trade_receivables / revenue_600 * period_days) if revenue_600 else 0
        ...
        def get_bank_breakdown(parent_code: str) -> list:
            ...
            for _, row in df.iterrows():
                ...
                if code.startswith(parent_code) and code != parent_code:
                    ...
                    if net_bal > 0:
                        name = aciklama_map.get(code, str(row.get("account_name", code)))
                        shares.append({"name": name, "balance": net_bal})
                        total_bal += net_bal
        ...
        try:
            ...
            prompt = (
                ...
                f"{summary}\n\n"
                f"### RAW DATA (Dynamically Extracted):\n"
                f"- **Revenue (600):** ₺{revenue_600:,.0f} | **COGS (620):** ₺{cogs_620:,.0f}\n"
                f"- **Operating Exp (630+631+632):** ₺{op_expenses:,.0f} | **EBITDA Proxy:** ₺{ebitda_proxy:,.0f}\n"
                f"- **Total Assets:** ₺{total_assets:,.0f} | **Current Assets:** ₺{current_assets:,.0f} | **Inventory:** ₺{inventory:,.0f}\n"
                f"- **Trade Recv (120+121):** ₺{trade_receivables:,.0f} | **Trade Pay (320+321):** ₺{trade_payables:,.0f}\n"
                ...
                f"- **Total Liabilities:** ₺{total_liab:,.0f} | **Total Bank Loans:** ₺{total_bank_loans:,.0f}\n"
                f"- **Fin. Expenses (780):** ₺{fin_exp_780:,.0f} | **POS Comm (780.01):** ₺{pos_780_01:,.0f}\n\n"
                f"### COMPETITOR BANK DISTRIBUTION:\n"
                f"- **102-BANKALAR (Deposits):** {fmt_shares(banks_102)}\n"
                f"- **300-BANKA KREDİLERİ KV (ST Loans):** {fmt_shares(banks_300)}\n"
                f"- **400-BANKA KREDİLERİ UV (LT Loans):** {fmt_shares(banks_400)}\n\n"
                f"### ADDITIONAL BANKING INTELLIGENCE GUIDELINES:\n"
                f"- CASH TRAPPING: Check Account 131 (Due from Shareholders). If it represents >5% of Total Assets, flag as 'Capital Leakage/Risk'.\n"
                ...
                f"- CASH CONVERSION: Analyze the Cash Conversion Cycle. If high, propose working capital financing (e.g., BCH/Rotatif Kredi).\n"
                f"- CROSS-SELL OPPORTUNITIES: Look at 'competitor_banks'. If Bank A has the majority of 300 (Loans) but we have the 102 (Deposits), suggest a loan buyout (refinancing).\n"
