...
async def run_swarm(request: RunSwarmRequest):
    try:
        ...
        state = {
            ...
            "donem_info": None,
            "db_financial_metrics": None, "db_product_flags": None, "db_meta": None,
        ...
        # Nereye eklendi: main.py içerisinde '/api/run-swarm' endpoint'inde, 
        # evaluation sonuçlandıktan hemen sonra.
        # ---------------------------------------------------------
       #stats = get_llm_stats()
       #total_calls = stats.get("total_llm_calls", 0)
       #total_tokens = stats.get("total_tokens_estimated", 0)
       #logger.info(f"{'-'*60}")
       #logger.info("  📊 GLOBAL LLM TOKEN USAGE (PIPELINE TOTAL)")
       #logger.info(f"{'-'*60}")
       #if total_calls > 0:
       ...
       #else:
       #    logger.info("  No LLM calls recorded during this execution.")
       #logger.info(f"{'-'*60}")
        # ---------------------------------------------------------
